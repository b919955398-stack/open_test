...
Release: 1.7.3 (`src` architecture, persistent PSS/E dispatched-case cache, transparent command/native PSS/E progress and PSCAD post-processing framework)
...
- Unit tests cover definitions, channel mappings, commands, profiles, grid math, Vslack/TOV calculations, PSS/E dispatch grouping/cache/invalidation/reuse, PSS/E-compatible SAV staging, state-aware halt handling, plain-text progress, legacy/new PSCAD cache handling, collision-free layout, DSL parsing, CSV/OUT/PSOUT reading, settling metrics, PSS/E/PSCAD replot pairing and declarative PSCAD analysis dispatch.
...
- Dispatched cases are staged under a short runtime path with a plain single-suffix ``.sav`` filename before Python publishes them into the persistent cache; skipped cases do not call ``PSSEHALT_2`` when PSS/E was never initialized.
- Each scenario preserves the established DYR/JSON/OUT/PNG/PDF/``_initialised.sav`` result group. Initialised SAVs use short PSS/E staging names and are copied to their full scenario names only after successful dynamic initialisation.
